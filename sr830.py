...
def connect(self, resource_name):
    try:
        ...
        self.parameters_sr830_time_constant.setCurrentIndex(3)
        
        # set_parameters(initialize=True)	
...
def read_parameters(self):   
    return {'time_constant':self.sr830.time_constant,
            'filter_slope':self.sr830.filter_slope,
            'input_config':self.sr830.input_config,
            'frequency':self.sr830.frequency,
            'input_coupling':self.sr830.input_coupling,
            'input_grounding':self.sr830.input_grounding,
            'sensitivity':self.sr830.sensitivity,
            'channel1':self.sr830.channel1,
            'channel2':self.sr830.channel2}
...
def set_parameters(self,parameter='all'):
    set_parameters_list = get_parameters(self)
            
    if parameter=='all':
        current_parameters = read_parameters(self)
        ...
        for i in range(len(set_parameters_list)):
            ...
            if set_parameters_list[parameter_name] != current_parameters[parameter_name]:
